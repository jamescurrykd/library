...
public class BookModiAndDelIFrame extends JInternalFrame {
	...
	private Object[][] getFileStates(List list){//取数据库中图书相关信息放入表格中
		...
		for(int i=0;i<list.size();i++){//遍历list
			...
			String booktypename=String.valueOf(MapPz.getMap().get(bookinfo.getTypeId()));//获得图书类别名称
	...
	public BookModiAndDelIFrame() {
		...
		Object[][] results=getFileStates(BookInfoDao.selectBookInfo());//获得书籍记录
		...
		bookTypeLabel.setHorizontalAlignment(SwingConstants.CENTER);//水平居中
		...
		publisherLabel.setHorizontalAlignment(SwingConstants.CENTER);//水平居中
		...
		pubDate= new JFormattedTextField();//创建日期输入框
		pubDate.setValue(myfmt.format(new java.util.Date()));//设置日期为当前系统时间
	...
	class TableListener extends MouseAdapter {
		public void mouseClicked(final MouseEvent e) {
			...
			bookType.setSelectedItem(typeids);//设置类别下拉框所选项
	...
	class UpdateBookActionListener  implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			if(ISBN.getText().length()!=2){//判断书籍编号的长度是否为13
			...
			int i=BookInfoDao.Updatebook(ISBNs, bookTypes, bookNames, writers, translators,
					publishers, Date.valueOf(pubDates), Double.parseDouble(prices));
			if(i==1){//如果返回更新记录数为1，表示修改成功
				JOptionPane.showMessageDialog(null, "修改成功");
				Object[][] results=getFileStates(BookInfoDao.selectBookInfo());//重新获得书籍信息
